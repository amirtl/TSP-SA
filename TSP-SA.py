...
def Energy(Gene, cities):
    return Find_Fitness(Gene, cities)
...
def SA(iteration, cities):
    n = len(cities)
    s = Create_Gene(n)
    for i in range(iteration):
        temperature = iteration/(i+1)
        s_new = neighbor(s[:])
        if Probability(Energy(s_new, cities), Energy(s, cities), temperature) > random.uniform(0,1):
            s = s_new
    
    print(Energy(s, cities))
    print(s)
